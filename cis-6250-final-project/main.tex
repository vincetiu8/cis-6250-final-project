...
\begin{itemize}
  \item
        ...
        can reach around 90\% top-1 accuracy on random labels even with explicit regularizers enabled.
  ...
  \item
        ...
        predictive signal).
  \item
        \textbf{Batch Normalization:} The authors note that Batch Normalization stabilizes training dynamics
        and can improve generalization in their experiments, but it does not prevent the network from memorizing noise.
\end{itemize}
...
linear models, the authors analyze (stochastic) gradient methods applied to the
overparameterized linear system $Xw=y$.

If initialized at zero, the iterates are always linear combinations of the data points $x_i$,
constraining the weight vector $w$ to lie in the span of the data. Consequently, in these linear settings
gradient-based optimization converges to the minimum $l_2$-norm solution among the infinite global minima.
This implies that the algorithm itself biases the solution toward a specific "simple" model, effectively solving the
kernel equation $XX^T\alpha = y$. In other words, gradient methods supply implicit regularization even when
...
\begin{itemize}
  ...
  \item
        \textbf{Nontrivial Guarantees:} While the empirical test error is small, the authors compute PAC-Bayes bounds
        in the range of \textbf{16--22\%} for their stochastic networks.
\end{itemize}
...
found by optimization controls the error.
...
\begin{enumerate}
  ...
  \item
        ...
        PAC-Bayes upper bound directly using gradient-based optimization. This creates a trade-off:
...
identify the distinguishing feature: \textbf{flat minima} (as witnessed by the ability to tolerate
weight perturbations while maintaining low error).

A minimum is considered "flat" if a large volume of weight space around it yields low error. In
the PAC-Bayes framework, flatness permits a wider posterior (larger perturbations) without
increasing the empirical error too much, which can lead to a substantially smaller information term
$\KL(Q\|P)$. This aligns with an MDL-style intuition: solutions that remain accurate under coarser
parameter precision are, in a precise sense, more compressible relative to the prior.
...
\begin{table}[h]
  ...
  \begin{tabular}{lccccc}
    ...
    \bottomrule
  \end{tabular}
  \caption{Comparison of generalization bounds on True vs. Random labels (binary MNIST). The large difference in
...
\begin{itemize}
  \item
        \textbf{Structured Data:} Optimization can locate solutions for which a relatively wide posterior
        (large perturbations) preserves low empirical error, yielding a much smaller KL term and a nonvacuous bound.
  \item
        \textbf{Random Labels:} To keep the stochastic network accurate on random labels, the learned posterior must be
        extremely concentrated (tiny effective perturbations), driving the KL divergence (and thus the bound) to become vacuous.
\end{itemize}

Thus, the PAC-Bayes objective acts as a quantitative witness to solution sensitivity under perturbations,
correctly separating the random-label regime from the structured-data regime despite both achieving near-zero training error.
...
Their results support two key takeaways: (i) many natural candidates (including several norm-only or sharpness-only
quantities) are insufficient to explain all observed behaviors once scale and width effects are accounted for, and
(ii) combinations of norms with perturbation-based robustness (including PAC-Bayes-motivated ``joint'' measures)
capture substantial portions of the observed trends, but still fail in some settings (e.g., for sufficiently large networks).
This refines Zhang et al.~\cite{zhang2017understanding}'s puzzle: identifying the right complexity measure is itself nontrivial,
and likely requires simultaneously incorporating scale, margin, and robustness.
...
\begin{enumerate}
  ...
  \item
        ...
        Fourier Features. They also report related behavior for $L_2$ boosting / AdaBoost in
...
infinitely many solutions that achieve zero training error. In linear regression / kernel-style
interpolation settings, a canonical inductive bias is to select a minimum-$\ell_2$-norm
interpolating solution (and gradient methods from small or zero initialization often recover it in
these settings). This bias can be written as:
...
expanded set can include interpolants with smaller norms than were available in the smaller space.
Since the optimizer prefers low-norm solutions in many overparameterized settings, expanding the
search space can allow it to find a "simpler" (lower norm) interpolant, thereby reducing test error.
This explains why the curve descends in the modern regime: the "effective complexity" (norm) can
decrease even as the "parameter complexity" ($d$) increases.
...
\begin{itemize}
  \item
        \textbf{Revisiting Zhang et al.:} Zhang et al.~\cite{zhang2017understanding} observed that networks can "memorize" (interpolate) random noise. Belkin et al.~\cite{belkin2019reconciling} clarify that interpolation itself is compatible with good generalization, and that test risk can vary non-monotonically around the interpolation threshold.
  \item
        \textbf{Validating Neyshabur et al.:} Neyshabur et al.'s~\cite{neyshabur2017exploring} argued that norms and robustness, not parameter counts alone, are central to capacity control. Belkin et al.~\cite{belkin2019reconciling} support this shift by emphasizing that function class capacity does not directly capture inductive bias, and that interpolating predictors in richer classes can generalize better.
  \item
        \textbf{Connecting to Dziugaite \& Roy:} Dziugaite \& Roy~\cite{dziugaite2017computing} identified perturbation-robustness (flatness, in a PAC-Bayes sense) as a key witness to generalization. In Belkin's framework, moving deeper into the overparameterized regime can make it easier for optimization to locate interpolants that match the problem's inductive bias, which is consistent with improved robustness in many settings.
\end{itemize}

Taken together, double descent inverts the classical wisdom.
Overparameterization is not always a liability to be bounded, but can be a feature that enables the finding of smoother (lower norm) interpolants.
The most severe generalization failures often occur near the interpolation threshold ($d \approx n$), while beyond it,
implicit regularization mechanisms and inductive bias can dominate, allowing test error to decrease even as training error remains near zero.
...
logistic loss has weights whose norm diverges while their \emph{direction} converges asymptotically
to the $l_2$ max-margin solution, effectively formalizing a max-margin implicit bias without explicit regularization.
\textbf{Lyu and Li (2020)}~\cite{lyu2019gradient}
extended this analysis to homogeneous deep neural networks, confirming that gradient flow maximizes
margin in deep architectures.
...
rigorous characterization of this phenomenon in high-dimensional linear regression. They identify
conditions on the covariance/eigenspectrum under which minimum-norm interpolating estimators can
still achieve low prediction error, mathematically validating portions of the interpolation regime.

\subsection{Operationalizing Flatness: Sharpness-Aware Minimization}
Dziugaite \& Roy~\cite{dziugaite2017computing} identified perturbation-robustness as a witness to
generalization and optimized PAC-Bayes objectives to obtain nonvacuous bounds, but they did not
propose a general-purpose deterministic training rule explicitly targeting flatness.
\textbf{Foret et al. (2021)}~\cite{foret2021sharpness} closed the loop between theory and practice
by introducing \textbf{Sharpness-Aware Minimization (SAM)}. By explicitly optimizing for both the
loss value and the flatness of the loss landscape (minimizing the maximum loss in a neighborhood),
SAM consistently improves generalization across state-of-the-art benchmarks, serving as strong
empirical validation of the robustness/flatness perspective.
...
(kernel) regime and the "rich" (feature learning) regime, arguing that important behaviors of deep learning can require escaping the kernel regime—a nuance that static complexity measures often miss.
...
central role of scale, norms, and robustness in candidate explanations, while emphasizing that no single measure yet explains all phenomena. Finally, Belkin
et al.~\cite{belkin2019reconciling} placed these insights into a unified framework, showing that
the "generalization puzzle" is closely tied to the behavior of interpolating solutions and inductive bias in the overparameterized regime.
