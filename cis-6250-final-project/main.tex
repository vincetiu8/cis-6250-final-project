...
\textbf{The classical framework fails spectacularly.}
...
\begin{enumerate}
  \item
        \textbf{The Puzzle:} Zhang et al.~\cite{zhang2017understanding} establish the empirical
        foundation by proving standard architectures can perfectly fit random labels.
  \item
        \textbf{The Existence Proof:} Dziugaite \& Roy~\cite{dziugaite2017computing} immediately
        counter the notion that theory is helpless by computing nonvacuous PAC-Bayes bounds.
  \item
        \textbf{The Mechanism:} Neyshabur et al.~\cite{neyshabur2017exploring} investigate the
        specific complexity measures (norm and sharpness) that drive this generalization.
  \item
        \textbf{The Unification:} Belkin et al.~\cite{belkin2019reconciling} provide the global
        phenomenological framework of "double descent" that reconciles Zhang's interpolation with
        the generalization analyzed by Dziugaite and Neyshabur.
\end{enumerate}
...
\begin{itemize}
  \item
        ...
        to prevent the memorization of random labels. For instance, Inception V3 fits random labels
        to over 90\% accuracy even with explicit regularizers enabled. This indicates that the
        "effective Rademacher complexity" of the hypothesis class remains high even under these
        constraints.
  \item
        ...
        with augmentation still fit random labels.
  \item
...
This theorem proves that simple depth-2 networks already possess universal finite-sample
expressivity as soon as the parameter count exceeds twice the sample size. The construction relies
on mapping the $d$-dimensional input points onto a single axis and utilizing the ReLU activation to
create a "gating" mechanism that fits specific targets. Crucially, this result contrasts with
uniform convergence bounds, which would typically require sample sizes exponential in the network
depth to guarantee learning. The "puzzle" of generalization, therefore, lies in understanding why,
among the vast space of parameters that achieve zero training error (including those that memorize
noise), SGD consistently finds solutions that perform well on unseen data.
...
Dziugaite \& Roy~\cite{dziugaite2017computing} refute the necessity of this pessimism by computing
...
\subsection{The PAC-Bayes framework as an optimization problem}
...
\begin{equation}
  \text{KL}(\hat{e}(Q, S_m) \| e(Q)) \leq \frac{\text{KL}(Q\|P) + \log(m/\delta)}{m-1}
\end{equation}
...
\begin{enumerate}
  ...
  \item
        ...
        the optimizer seeks weights $w$ with low error (minimizing the first term) while
        simultaneously inflating the variance $s$ to stay close to the prior $P$ (minimizing the KL
        penalty).
  \item
        ...
        fact that SGD does not travel infinitely far from its starting point and helps break the
        permutation symmetries inherent in neural networks.
\end{enumerate}
...
prior, resulting in a low $\text{KL}(Q\|P)$ term.
...
\begin{table}[h]
  ...
  \begin{tabular}{lccccc}
    ...
    True Labels (T-1200)  & 2.38M           & 3.5\%                   & \textbf{22.3\%}           & 8,558                  \\
    \midrule
    Random Labels (R-600) & 472K            & 50.3\%                  & \textbf{Vacuous (>100\%)} & \textbf{201,131}       \\
    \bottomrule
  \end{tabular}
  \caption{Comparison of generalization bounds on True vs. Random labels. The massive difference in
    KL divergence explains the generalization gap.}
\end{table}
...
A critical contribution of this work is the rigorous critique of "sharpness" as a standalone
...
For a network with ReLU activations (which are non-negative homogenous, i.e., $\text{ReLU}(\alpha
...
Crucially, empirical evaluations reveal that the \textbf{$L_2$ (Frobenius) norm} and the
\textbf{$L_1$-path norm} provide bounds that are essentially \textbf{independent of network width}.
This offers a theoretical justification for the "finite sample expressivity" observations in Zhang
et al.\cite{zhang2017understanding}: adding more hidden units increases the potential parameter
space (VC dimension), but it does not necessarily increase the norm-based complexity of the
solution actually found by SGD. The optimization algorithm implicitly constrains the solution to a
low-norm subspace, rendering the vast number of excess parameters benign.
...
The results confirm that while spectral norm fails to predict the generalization gap in these
scenarios (often remaining constant despite degrading test performance), the \textbf{PAC-Bayes
 measure (combining norm and sharpness)} successfully correlates with test error. This explains
Zhang et al.~\cite{zhang2017understanding}'s findings:

\begin{itemize}
  \item
        \textbf{On Random Labels:} SGD finds a solution that is "sharp" relative to its norm. To
        fit noise, the weights must be large or the minimum extremely narrow, causing the PAC-Bayes
        ratio $\frac{\|w\|}{\sigma}$ to explode.
  \item
        \textbf{On True Labels:} SGD finds a solution with low norm and high robustness (flatness),
        yielding a tight, nonvacuous bound.
\end{itemize}
...
bounds and norms), they left open the question of why the optimization process doesn't explode when
passing through the regime where the model capacity exactly matches the dataset size ($n \approx
 d$). Belkin et al.'s 2019 paper "Reconciling Modern Machine Learning Practice and the
Bias-Variance Trade-off"~\cite{belkin2019reconciling} provides the overarching phenomenological
framework that unifies these observations into a single coherent narrative.
...
learning regime. They demonstrate that the test error behavior is non-monotonic and distinct across
...
\begin{enumerate}
  ...
  \item
        ...
        of data points ($d \approx n$), the risk peaks, often dramatically. Here, there is
        effectively only one unique solution that fits the training data. Because the model has
        just barely enough capacity to fit the random noise in the labels, the resulting function
        must oscillate wildly, leading to massive coefficient sizes, high variance, and poor
        generalization. Mathematically, this corresponds to the regime where the condition number
        of the covariance matrix explodes, causing numerical instability and sensitivity to noise.
  \item
...
Why does increasing capacity reduces error in the modern regime? The authors argue that the
...
\subsection{Why Overparameterization Helps}

Belkin et al.~\cite{belkin2019reconciling} essentially invert the classical wisdom.
...
on rigorousizing these phenomenological observations and translating them into practical
algorithms.
...
et al.~\cite{belkin2019reconciling} placed these insights into a unified phenomenological
framework, showing that the "generalization puzzle" is actually a predictable property of
minimum-norm interpolants in the overparameterized regime.
