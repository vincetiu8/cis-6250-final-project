...
\title{
    ...
    A Synthesis of Foundational Theoretical Perspectives
}
...
\begin{itemize}
  \item
        ...
        to around 90\% accuracy even with explicit regularizers enabled. This indicates that the
...
Hessian of the loss function is degenerate and identical for \textit{all} globally optimal solutions.
...
expressivity with $2n + d$ parameters. The construction relies
...
\begin{enumerate}
  ...
  \item
        ...
        the optimizer seeks weights $w$ with low error while choosing the variance $s$ to trade off
        empirical robustness under perturbations against the KL penalty.
  \item
        ...
        permutation symmetries inherent in neural networks—the property that reordering neurons
        within a hidden layer preserves the network's function.
\end{enumerate}
...
Neyshabur et al.~\cite{neyshabur2017exploring} reinforce the consensus established by Zhang et
...
Crucially, empirical evaluations suggest that the \textbf{$L_2$ (Frobenius) norm} and the
\textbf{$L_1$-path norm} can remain relatively stable as network width increases. This offers a
theoretical justification for the "finite sample expressivity" observations in Zhang
...
The results suggest that while spectral norm can fail to predict the generalization gap in these
scenarios (often remaining constant despite degrading test performance), joint measures (such as
PAC-Bayes-style combinations of norm and sharpness) can track the trend in many cases, but can also
fail in some settings. This aligns with Zhang et al.~\cite{zhang2017understanding}'s findings:

\begin{itemize}
  \item
        \textbf{On Random Labels:} SGD can find a solution that is "sharp" relative to its norm. To
        fit noise, the weights must be large or the minimum extremely narrow, causing the PAC-Bayes
        ratio $\frac{\|w\|}{\sigma}$ to explode.
  \item
        \textbf{On True Labels:} SGD can find a solution with low norm and high robustness (flatness),
...
\begin{enumerate}
  ...
  \item
        ...
        of data points ($d \approx n$), the risk peaks, often dramatically. Here, the set of
        interpolating solutions can be particularly ill-conditioned or unstable. Because the model has
