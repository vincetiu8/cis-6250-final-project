...
The authors show that the PAC-Bayes generalization bound can be decomposed into two competing
terms:

\begin{equation}
  \text{Generalization Error} \lesssim \underbrace{\mathbb{E}_{\nu}[L(w+\nu)]
    - L(w)}_{\text{Expected Sharpness}}
  + \underbrace{4\sqrt{\frac{1}{m}\text{KL}(w+\nu \| P)}}_{\text{Information Penalty}}
\end{equation}

When the perturbation $\nu$ is Gaussian with variance $\sigma^2$ and the prior $P$ is a standard
Gaussian centered at initialization, the KL divergence term becomes proportional to
$\frac{\|w\|_2^2}{\sigma^2}$. This synthesis reveals the mechanism of generalization as a balance:

\begin{itemize}
  \item
        \textbf{The Trade-off:} To minimize the bound, one acts on the ratio. A large perturbation
        variance $\sigma$ (indicating a flat minimum) is desirable to minimize the KL term, but
        only if the loss remains low (high robustness). Conversely, having a small weight norm
        $\|w\|_2$ allows one to tolerate sharper minima (smaller $\sigma$) without paying a massive
        information penalty.
  \item
        \textbf{The Conclusion:} Generalization is not determined by sharpness alone (which is
        scale-variant), nor by norm alone, but by the \textbf{ratio of the weight norm to the
          flatness of the minimum}. This unified view incorporates both Dziugaite \&
        Roy~\cite{dziugaite2017computing}'s "flatness" and classical "margin/norm" theory.
\end{itemize}
...
\begin{itemize}
  \item
        Many natural candidates (including several norm-only or sharpness-only quantities) are insufficient
        to explain all observed behaviors once scale and width effects are accounted for.
  \item
        Combinations of norms with perturbation-based robustness (including PAC-Bayes-motivated ``joint'' measures)
        capture substantial portions of the observed trends, but still fail in some settings
        (e.g., for sufficiently large networks).
\end{itemize}
...
(kernel) regime and the "rich" (feature learning) regime, arguing that important behaviors of deep
learning can require escaping the kernel regime—a nuance that static complexity measures often miss.
