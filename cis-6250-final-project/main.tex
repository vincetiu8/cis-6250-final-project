...
\begin{enumerate}
  ...
  \item
        ...
        Because the model has just barely enough capacity to fit the random noise in the labels,
        the resulting function must oscillate wildly, leading to massive coefficient sizes, high
        variance, and poor generalization. Mathematically, this corresponds to the regime where the
        condition number of the covariance matrix explodes, causing numerical instability and
...
to the $l_2$ max-margin solution (the minimum norm separator direction), effectively formalizing a
max-margin implicit bias without explicit regularization. \textbf{Lyu and Li (2020)}~\cite{lyu2019gradient}
...
still achieve low prediction error (roughly, when noise can be absorbed in many weak-variance
directions without corrupting the signal-aligned directions), mathematically validating Belkin's
